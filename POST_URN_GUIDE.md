...
async function enviarURN() {
  ...
  try {
    const response = await fetch(
      "http://localhost:8081/api/models/upload-urn",
      {
        method: "POST",
        headers: {
    ...
    if (resultado.success) {
      ...
      alert(
        `Modelo "${resultado.model.name}" adicionado com ID: ${resultado.model.id}`
      );
    } else {
      ...
      <form id="formURN">
        ...
        <div class="form-group">
          ...
          <textarea
            ...
            name="description"
            rows="2"
            placeholder="Descrição do modelo BIM"
          ></textarea>
        </div>
              ...
              resultadoDiv.innerHTML = `
                        <div class="resultado sucesso">
                            ...
                            <p><strong>Nome:</strong> ${
                              resultado.model.name
                            }</p>
                            <p><strong>Status:</strong> ${
                              resultado.model.status
                            }</p>
                            <p><strong>Data:</strong> ${new Date(
                              resultado.model.uploadedAt
...
function FormularioURN() {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {sucesso && (
        <div style={{ color: "green" }}>✅ URN enviada: {sucesso.name}</div>
      )}
    </form>
  );
}
...
```javascript
const response = await fetch(
  `http://localhost:8081/api/models/custom/${modelId}`
);
