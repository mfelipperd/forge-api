...
async function fixUrnStrategy() {
  ...
  try {
    ...
    for (const model of models) {
      ...
      if (model.name.includes('BR6-CSFAIP') || model.fileName === 'BR6-CSFAIP.IFC') {
        ...
        if (model.urn === REAL_URN) {
          console.log('   🔄 Removendo URN real (não corresponde a este modelo)...');
          
          // Gerar uma URN fake única baseada no nome do arquivo
          const fakeBuName = model.fileName || model.name.replace(/[^a-zA-Z0-9]/g, '-');
