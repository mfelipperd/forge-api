    ...
    models.forEach((model, index) => {
      ...
      if (model.urn) {
        try {
          ...
          console.log(`   Contém 'forge-viewer-models/': ${decodedUrn.includes('forge-viewer-models/')}`);
        } catch (error) {
      ...
      if (model.base64Urn) {
        try {
          const decodedBase64 = Buffer.from(model.base64Urn, 'base64').toString('utf-8');
          console.log(`   Base64URN decodificada: ${decodedBase64}`);
          console.log(`   Contém 'forge-viewer-models/': ${decodedBase64.includes('forge-viewer-models/')}`);
        } catch (error) {
